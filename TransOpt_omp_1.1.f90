...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!Change log
!!2020.7  TransOpt 1.0 Computational Materials Science 186,110074 (2021)
!!2021.12 TransOpt 1.1 Edef(nspin) consider the different deformation potentials
!for each spin
...
real(dp),save,allocatable::cond(:,:,:),condinv(:,:,:),cond_tau(:,:,:),cond_tau_inv(:,:,:),mu(:,:,:),mu_tau(:,:,:),kappa(:,:,:),kappa_tau(:,:,:),condtemp(:,:,:),condtemp_tau(:,:,:),mutemp(:,:,:),mutemp_tau(:,:,:),kappatemp(:,:,:),kappatemp_tau(:,:,:),vk2temp(:,:,:),vk2temp_tau(:,:,:),seebeck(:,:,:),seebeck_tau(:,:,:),ke(:,:,:),ke_tau(:,:,:),Edef(:),Edef_origin(:)
...
do ispin=1,nspin
  C(ispin)=2*pi*kb*Edef(ispin)*Edef(ispin)*e*e*e/(elastic*1E9)/hbar
  irtau(ispin,:,:)=1/(irtau(ispin,:,:)*C(ispin)*T/volume)
enddo
...
if(nspin==1) then
  ...
  write(38,"(A,F10.4,A,F10.4,A,2F10.4,A,F10.4)") "Gauss_smearing= ",sigma,"   T(K)= ",T, "   Edef(eV)= ",Edef(:),"   Elastic(GPa)= ",elastic
endif
write(38,*)""
!
