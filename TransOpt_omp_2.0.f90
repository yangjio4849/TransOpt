...
!!!!!!!!  GROUPVEC EIGENVAL POSCAR SYMMETRY TransOpt.input OUTCAR       !!!!!!!!
!!!!!!!!input files of derivation method:                               !!!!!!!!
!!!!!!!!  GVEC EIGENVAL POSCAR SYMMETRY TransOpt.input OUTCAR           !!!!!!!!
!!!!!!!!                                                                !!!!!!!!
!!!!!!!!control.in is NOT necessary anymore, for version 2.0 and later  !!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!Change log
!!2020.7  TransOpt 1.0 
...
!!2021.12 TransOpt 1.1 
!Edef(nspin) consider the different deformation potentials for each spin

!!2022.3  TransOpt 2.0 
!Add the ionized impurity scattering, following Scientific Reports 6, 19968 & JACS 137, 12929. 
!The function of readTAU, i.e., read the original TAU file has been discontinued. 
!control.in has been merged with TransOpt.input (formerly finale.input). Add a number choice of scattering option.
!The deformation potential has been splitted into EdefVB(:) and EdefCB(:). 
...
real(dp),save,allocatable::irvk(:,:,:,:),vk(:,:,:,:),alltau(:,:,:)
...
real(dp),save,allocatable::cond(:,:,:),condinv(:,:,:),cond_tau(:,:,:),cond_tau_inv(:,:,:),mu(:,:,:),mu_tau(:,:,:),kappa(:,:,:),kappa_tau(:,:,:),condtemp(:,:,:),condtemp_tau(:,:,:),mutemp(:,:,:),mutemp_tau(:,:,:),kappatemp(:,:,:),kappatemp_tau(:,:,:),vk2temp(:,:,:),vk2temp_tau(:,:,:),seebeck(:,:,:),seebeck_tau(:,:,:),ke(:,:,:),ke_tau(:,:,:),EdefVB(:),EdefCB(:)
real(dp),save:: epsilon_r,nimpurity,n0,zion           ! for ionized impurity scattering
real(dp),save:: estart,de,emu,elastic,sigma
...
!!2022.2 The next three lines are not necessary anymore
!logical alive
!logical,save:: readTAU
!real(dp),save:: sigma_origin,T_origin,elastic_origin
!
...
allocate(dimens(3))
!!!!!!!!!!!!read TransOpt.input 
open(unit=37,file="TransOpt.input",status="old",iostat=eof)
if(eof==0) then
  ...
  stop "There is no input file TransOpt.input"
endif
...
if (ifderivation) then
  ...
  if(eof/=0) stop "Need OUTCAR to determine whether the calculation is spin-orbit coupling!"
  do while(.TRUE.)
    read(38,*)nomeaning
...
if (ifderivation) then
  ...
  write(*,"(A)")"!!!!!!Attention, you can only adopt the momentum matrix method in no-soc calculation!!!!!"
...
!allocate(alltau(nspin,nband,nk),scattering_ep(nspin,nband,nk),scattering_imp(nspin,nband,nk),irtau(nspin,nband,nirk),vk2temp(3,3,nspin),vk2temp_tau(3,3,nspin))
allocate(alltau(nspin,nband,nk),vk2temp(3,3,nspin),vk2temp_tau(3,3,nspin))
allocate(cond(3,3,nspin),condinv(3,3,nspin),cond_tau(3,3,nspin),cond_tau_inv(3,3,nspin),condtemp(3,3,nspin),condtemp_tau(3,3,nspin),mu(3,3,nspin),mu_tau(3,3,nspin),mutemp(3,3,nspin),mutemp_tau(3,3,nspin),kappa(3,3,nspin),kappa_tau(3,3,nspin),kappatemp(3,3,nspin),kappatemp_tau(3,3,nspin),seebeck(3,3,nspin),seebeck_tau(3,3,nspin),ke(3,3,nspin),ke_tau(3,3,nspin),dos(nspin),dostemp(nspin),dosint(nspin),dosinttemp(nspin),dosvk(nspin),dostau(nspin),dosvktemp(nspin),dostautemp(nspin),nCBB(nspin),sis(nspin),EdefVB(nspin),EdefCB(nspin))


!! Continue to get the rest input info from TransOpt.input.
read(37,"(A80)") input
read(input,*,iostat=eof)efermi,estart,de,nemu,T,nCBB(:),sis(:),carriertotale
carrier=.FALSE.
if(eof==0) carrier=.TRUE.
if (scattering>0) then
  read(37,*) EdefVB(:),EdefCB(:),elastic,sigma   !  in the unit eV, GPa, eV
  if (scattering==2) read(37,*,iostat=eof) zion,nimpurity,n0,epsilon_r
endif
!  Edef=10
!  elastic=100
!  sigma=0.5
...
!!2019.1 no more TAU_fullmesh, only TAU. All the full mesh info, e.g., alltau,
!can be from irtau and corresponding(nk)
!!2022.2 the readTAU module is not necessary anymore!
...
!    close(38)
!    do ispin=1,nspin
!      irtau(ispin,:,:)=irtau(ispin,:,:)*1E-14/elastic_origin*Edef_origin(ispin)*Edef_origin(ispin)*T_origin*elastic/Edef(ispin)/Edef(ispin)/T
 !   enddo
 ! else
 !   call getTAU()
!  endif
...
if (scattering>0) then
!! 2022.2 
  write(*,*) "Starting evaluation of the relaxation time"
  call getTAU()
endif

!! copy the irtau info to alltau
!! 2022.2 Not necessary! Will be done in getTAU()
!  do ik=1,nk
...
SUBROUTINE getTAU()
!! 2022.2 do all the calculations of scattering rates on the full mesh, not
!irtau anymore. This is due to the impurity scattering needs k-k'
use variables
use variable
implicit none
real(dp) gauss,C_VB(nspin),C_CB(nspin),LD,imp
real(dp) scattering_ep(nspin,nband,nk),scattering_ep_irk(nspin,nband,nirk),scattering_imp(nspin,nband,nk)
integer ikprime,ibandprime
real(dp) deltaenergy,kkprim
real(dp) kptcar(3,nk)

! constant for acoustic e-p scattering
do ispin=1,nspin
  C_VB(ispin)=2*pi*kb*EdefVB(ispin)*EdefVB(ispin)*e*e*e/(elastic*1E9)/hbar*T/volume
  C_CB(ispin)=2*pi*kb*EdefCB(ispin)*EdefCB(ispin)*e*e*e/(elastic*1E9)/hbar*T/volume
!  irtau(ispin,:,:)=irtau(ispin,:,:)*C(ispin)*T/volume
enddo

! constant for ionized impurity scattering nimpurity and n0 are in the unit of 10^20 cm-3
if(scattering==2) imp=2*pi/hbar*nimpurity*1E26*zion*zion*e**4/volume/(epsilon_r*epsilon_0)**2
if(scattering==2) LD=sqrt(epsilon_r*epsilon_0*kb*T*e/(n0*1E26)/e/e)

scattering_ep=0
scattering_ep_irk=0
scattering_imp=0

if(scattering==1) then
...
! assume deltaenergy> 5 to skip the loop through cycle
            ...
            scattering_ep_irk(ispin,iband,iirk)=scattering_ep_irk(ispin,iband,iirk)+gauss
          enddo
        enddo
      enddo
    enddo
...
!$OMP end parallel do
  do ik=1,nk
    scattering_ep(:,:,ik)=scattering_ep_irk(:,:,corresponding(ik))
  enddo
endif


if (scattering==2) then
  kptcar=kpt
!2022.2 convert kpt for direct coordinates to cartesian coordinates
  do ik=1,nk
    call matmulonethree(kptcar(:,ik),vecb,kpttemp)
    kptcar(:,ik)=kpttemp
  enddo
...
!$OMP parallel do
  do ik=1,nk
    do iband=1,nband
      do ispin=1,nspin
        do ikprime=1,nk
          kkprim=DOT_PRODUCT(kptcar(:,ik)-kptcar(:,ikprime),kptcar(:,ik)-kptcar(:,ikprime))
          do ibandprime=1,nband
! assume deltaenergy> 5 to skip the loop through cycle
            deltaenergy=abs((bandenergy(ispin,ibandprime,ikprime)-bandenergy(ispin,iband,ik))/sigma)
            if(deltaenergy > 5.0) then
              cycle
            else
              gauss=exp(deltaenergy*deltaenergy/(-2.0))/sqrt(2*pi)/(sigma*e)/nk
            endif
            scattering_ep(ispin,iband,ik)=scattering_ep(ispin,iband,ik)+gauss
            scattering_imp(ispin,iband,ik)=scattering_imp(ispin,iband,ik)+gauss/(1.0/LD/LD+kkprim)**2
          enddo
        enddo
      enddo
    enddo
...
!! 2022.2 both the two scattering mechanisms multiplied by constants
do ik=1,nk
  do ispin=1,nspin
    do iband=1,nCBB(ispin)-1
      scattering_ep(ispin,iband,ik)=scattering_ep(ispin,iband,ik)*C_VB(ispin)
    enddo
    do iband=nCBB(ispin),nband
      scattering_ep(ispin,iband,ik)=scattering_ep(ispin,iband,ik)*C_CB(ispin)
    enddo
  enddo
enddo

if(scattering==2) scattering_imp=scattering_imp*imp


do ik=1,nk
  do iband=1,nband
    do ispin=1,nspin
      alltau(ispin,iband,ik)=1.0/(scattering_ep(ispin,iband,ik)+scattering_imp(ispin,iband,ik))
    enddo
  enddo
enddo
...
!!2019.1 no more TAU_fullmesh, only TAU. All the full mesh info, e.g., alltau,
!can be from irtau and corresponding(nk)
!!2022.2 the title of TAU is not useful anymore.
if(nspin==1) then
  if(scattering==2) then 
    write(38,"(A,F10.4,A,F10.4,A,2F10.4,A,F10.4,A,F10.4,A,F10.4,A,F10.4,A,F10.4)") "Gauss_smearing= ",sigma,"   T(K)= ",T, "   Edef(eV)= ",EdefVB(:),EdefCB(:),"   Elastic(GPa)= ",elastic,"  Zion=",zion,"  n_impurity(10^20 cm-3)=", nimpurity,"  n_carrier(10^20 cm-3)=",n0,"  relative dielectric constant=", epsilon_r
  else
    write(38,"(A,F10.4,A,F10.4,A,2F10.4,A,F10.4)") "Gauss_smearing= ",sigma,"   T(K)= ",T, "   Edef(eV)= ",EdefVB(:),EdefCB(:),"   Elastic(GPa)= ",elastic
  endif
else
  if(scattering==2) then 
    write(38,"(A,F10.4,A,F10.4,A,4F10.4,A,F10.4,A,F10.4,A,F10.4,A,F10.4,A,F10.4)") "Gauss_smearing= ",sigma,"   T(K)= ",T, "   Edef(eV)= ",EdefVB(:),EdefCB(:),"   Elastic(GPa)= ",elastic,"  Zion=",zion,"  n_impurity(10^20 cm-3)=", nimpurity,"  n_carrier(10^20 cm-3)=",n0,"  relative dielectric constant=", epsilon_r
  else
    write(38,"(A,F10.4,A,F10.4,A,4F10.4,A,F10.4)") "Gauss_smearing= ",sigma,"   T(K)= ",T, "   Edef(eV)= ",EdefVB(:),EdefCB(:),"   Elastic(GPa)= ",elastic
  endif
endif
write(38,*)""
!
...
!  write(38,"(4F15.8)")irkpt(:,iirk),irkwpt(iirk)
  write(38,"(4F15.8)")kpt(:,ik),1.0/nk
  do iband=1,nband
    if (nspin==1) then
!      write(38,"(I5,F12.6,A)")iband,irtau(:,iband,iirk)*1E14, "  *10^-14 s"
      write(38,"(I5,F12.6,A)")iband,alltau(:,iband,ik)*1E14, "  *10^-14 s"
    else
!      write(38,"(I5,2F12.6,A)")iband,irtau(:,iband,iirk)*1E14, "  *10^-14 s"
      write(38,"(I5,2F12.6,A)")iband,alltau(:,iband,ik)*1E14, "  *10^-14 s"
    endif
  enddo
  write(38,*)" "
enddo
...
do ik=1,nk
  ...
  do iband=1,nband
    do ispin=1,nspin
      ...
      if (scattering>0) dostautemp(ispin)=dostautemp(ispin)+defer*alltau(ispin,iband,ik)
  ...
  do ispin=1,nspin
    ...
    if (scattering>0) dostau(ispin)=dostau(ispin)+dostautemp(ispin)/(1.0*nk)
...
mu=0.0
kappa=0.0
if (scattering>0) cond_tau=0.0
if (scattering>0) mu_tau=0.0
if (scattering>0) kappa_tau=0.0
do ik=1,nk
  condtemp=0.0
  mutemp=0.0
  kappatemp=0.0
  if (scattering>0) condtemp_tau=0.0
  if (scattering>0) mutemp_tau=0.0
  if (scattering>0) kappatemp_tau=0.0
  do iband=1,nband
    do ispin=1,nspin
      ...
      do i=1,3
        do j=1,3
          ...
          if (scattering>0) vk2temp_tau(i,j,ispin)=vk2temp(i,j,ispin)*alltau(ispin,iband,ik)
          if (scattering>0) condtemp_tau(i,j,ispin)=condtemp_tau(i,j,ispin)+vk2temp_tau(i,j,ispin)
          if (scattering>0) mutemp_tau(i,j,ispin)=mutemp_tau(i,j,ispin)+vk2temp_tau(i,j,ispin)*(bandenergy(ispin,iband,ik)-fermicur)*e
          if (scattering>0) kappatemp_tau(i,j,ispin)=kappatemp_tau(i,j,ispin)+vk2temp_tau(i,j,ispin)*(bandenergy(ispin,iband,ik)-fermicur)**2*e*e
        enddo
      enddo
    enddo
  enddo
  do ispin=1,nspin
    do i=1,3
      do j=1,3
        ...
        if (scattering>0) cond_tau(i,j,ispin)=cond_tau(i,j,ispin)+condtemp_tau(i,j,ispin)/(1.0*nk)
        if (scattering>0) mu_tau(i,j,ispin)=mu_tau(i,j,ispin)+mutemp_tau(i,j,ispin)/(1.0*nk)
        if (scattering>0) kappa_tau(i,j,ispin)=kappa_tau(i,j,ispin)+kappatemp_tau(i,j,ispin)/(1.0*nk)
      enddo
    enddo
  enddo
enddo
...
if (scattering>0) cond_tau=cond_tau/volume
if (scattering>0) mu_tau=mu_tau/(-1*e*T*volume)
if (scattering>0) kappa_tau=kappa_tau/(e*e*T*volume)
do i=1,3
  if (.NOT.dimens(i)) then 
    do j=1,3
      ...
      if (scattering>0) cond_tau(i,j,ispin)=1E-30
      if (scattering>0) cond_tau(j,i,ispin)=1E-30
      enddo
    enddo
  endif
enddo
...
seebeck=0.0
if (scattering>0) seebeck_tau=0.0
ke=0.0
if (scattering>0) ke_tau=0.0
do ispin=1,nspin
  do i=1,3
    do j=1,3
!      do k=1,3
...
!      enddo
      ...
      if (scattering>0) seebeck_tau(i,j,ispin)=cond_tau_inv(i,j,ispin)*mu_tau(i,j,ispin)
      ke(i,j,ispin)=kappa(i,j,ispin)-T*mu(i,j,ispin)**2*condinv(i,j,ispin)
      if (scattering>0) ke_tau(i,j,ispin)=kappa_tau(i,j,ispin)-T*mu_tau(i,j,ispin)**2*cond_tau_inv(i,j,ispin)
    enddo
  enddo
enddo
